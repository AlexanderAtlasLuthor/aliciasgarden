...
// src/lib/http.ts
function jsonOk(c, data, status = 200) {
...
// src/services/ai.ts
async function generateReply(messages, env) {
  if (!env.AI_API_KEY) {
...
async function sendChatMessage(message, threadId, env) {
  ...
  if (!effectiveThreadId) {
    const { data: thread, error: createThreadError } = await supabase.from("chat_threads").insert({
  ...
  const { data: historyRows, error: historyError } = await supabase.from("chat_messages").select("role, content, created_at").eq("thread_id", effectiveThreadId).order("created_at", { ascending: false }).limit(20);
  if (historyError) {
    throw new Error(`Failed to load chat history: ${historyError.message}`);
  }
  const history = historyRows.slice().reverse().map((row) => ({
    role: row.role,
    content: row.content
  }));
  const aiMessages = [SYSTEM_PROMPT, ...history];
  let reply;
  try {
...
__name(asOptionalString, "asOptionalString");
...
chatRoutes.post("/chat/send", async (c) => {
  ...
  const threadId = rawThreadId?.trim() ? rawThreadId.trim() : void 0;
  try {
...
chatRoutes.get("/chat/threads", async (c) => {
  try {
    ...
    const { data: threads, error } = await supabase.from("chat_threads").select("*").eq("profile_id", c.env.PROFILE_ID).order("created_at", { ascending: false }).limit(50);
    if (error) {
...
chatRoutes.get("/chat/threads/:id", async (c) => {
  ...
  try {
    const supabase = getSupabase(c.env);
    const { data: messages, error } = await supabase.from("chat_messages").select("*").eq("thread_id", threadId).order("created_at", { ascending: true });
    if (error) {
...
var plantsRoutes = new Hono2();
plantsRoutes.get("/plants", async (c) => {
  try {
    const supabase = getSupabase(c.env);
    const { data: plants, error } = await supabase.from("plants").select("*").eq("profile_id", c.env.PROFILE_ID).order("created_at", { ascending: false });
    if (error) {
...
plantsRoutes.post("/plants", async (c) => {
  ...
  try {
    const supabase = getSupabase(c.env);
    const { data: plant, error } = await supabase.from("plants").insert(payload).select("*").single();
    if (error) {
...
// .wrangler/tmp/bundle-x5Tuun/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-x5Tuun/middleware-loader.entry.ts

...
import { sendPlantChatMessage } from "@/lib/api"
...
export default function PlantToniPage() {
  ...
  const storageKey = plantId ? `ag_plant_thread_${plantId}` : ""
  ...
  const sendMessage = async (message: string, appendUserMessage: boolean) => {
    ...
    try {
      const result = await sendPlantChatMessage(plantId, message)

      setThreadId(result.thread_id)
      if (storageKey) {
        localStorage.setItem(storageKey, result.thread_id)
      }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmedMessage || isSending || !plantId) {
      return
    }
      ...
      >
        <section className="space-y-2">
          ...
          <div className="flex items-center justify-between gap-3">
            <h1 className="text-primary text-2xl font-semibold tracking-tight">Toni está viendo esta planta 🌿</h1>
            <button
              type="button"
          ...
          <p className="text-secondary text-sm">Haz preguntas sobre esta planta y Toni responderá con su contexto.</p>
        </section>
        ...
        <div className="ag-container">
          <div className="relative overflow-hidden rounded-2xl border border-white/16 bg-[#1b4034] p-2">
            <div className="relative flex items-center gap-2">
              ...
              <Button
                type="submit"
                disabled={isSending || !input.trim() || !plantId}

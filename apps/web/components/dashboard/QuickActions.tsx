...
export default function QuickActions() {
  return (
    <section>
      <GlassSurface className="ag-emerald-plate space-y-4 p-4 md:p-5" variant="medium">
        ...
        <div className="grid grid-cols-2 gap-2.5 md:grid-cols-4">
          {ACTIONS.map((action) => (
            <Link
              key={action.label}
              href={action.href}
              className="ag-emerald-plate group flex flex-col items-center gap-1.5 rounded-2xl border border-white/10 px-3 py-3.5 text-center backdrop-blur-sm transition-all duration-200 hover:border-white/15"
            >
